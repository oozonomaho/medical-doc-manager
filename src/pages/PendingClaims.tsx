...
const PendingClaims: React.FC = () => {
  ...
  const filteredPatients = useMemo(() => {
    let filtered = activePatients.filter(patient => {
      ...
      const completionDate = patient.selfSupportCertificate?.completionDate;
      const status = patient.selfSupportCertificate?.status;
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredPatients.map((patient) => (
                <tr key={patient.id}>
                  ...
                  <td className="px-6 py-4">
  {patient.selfSupportCertificate?.completionDate ? 
    new Date(patient.selfSupportCertificate.completionDate).toLocaleDateString('ja-JP') : 
    '-'
  }
</td>
                  <td className="px-6 py-4">
