...
const MedicalCertificates: React.FC = () => {
  ...
  const getCertificateRows = (patient: any) => {
    ...
    if (patient.selfSupportCertificate?.progress?.requestSent) {
      rows.push({
        ...
        staff: '',
        status: '未',
        applicationDate: patient.selfSupportCertificate.applicationDate
      });
    }
  
    if (patient.disabilityCertificate?.progress?.requestSent) {
      rows.push({
        ...
        staff: '',
        status: '未',
        applicationDate: patient.disabilityCertificate.applicationDate
      });
    }
  
    if (patient.pensionStatus?.progress?.requestSent) {
      rows.push({
    ...
    const handleRenewal = () => {
      const self = patient.selfSupportCertificate;
      const dis = patient.disabilityCertificate;
      ...
      if (canSimultaneousRenew) {
        ...
        rows.push({
          ...
          type: '自立支援＋手帳',
          renewalType: '更新',
          deadline: selfValidUntil,
          needsCertificate: needsSelf || needsDis,
          priority: '',
          staff: '',
          status: '未'
        });
      } else {
        if (self?.hasSupport && selfValidUntil) {
          rows.push({
            ...
            renewalType: '更新',
            deadline: selfValidUntil,
            needsCertificate: needsMedicalCertificate(self, '自立支援'),
            priority: '',
            staff: '',
            status: '未'
          });
        ...
        if (dis?.hasDisability && disValidUntil) {
          rows.push({
            id: `${patient.id}-disability-renewal`,
            patient,
            type: '手帳',
            renewalType: '更新',
            deadline: disValidUntil,
            needsCertificate: needsMedicalCertificate(dis, '手帳'),
            priority: '',
            staff: '',
            status: '未'
          });
      ...
      if (pen?.hasPension && penValidUntil) {
        rows.push({
          id: `${patient.id}-pension-renewal`,
          patient,
          type: '年金',
          renewalType: '更新',
          deadline: penValidUntil,
          needsCertificate: needsMedicalCertificate(pen, '年金'),
          priority: '',
          staff: '',
          status: '未'
        });
