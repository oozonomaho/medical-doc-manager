...
const DeadlineManagement: React.FC = () => {
  ...
  const getNextAction = (certificate: any) => {
  ...
  const getCertificateType = (patient: any, type: DeadlineType) => {
    switch (type) {
      case '自立支援':
        return patient.selfSupportCertificate?.hasSupport ? '自立支援' : '不要';
      case '障害者手帳':
        return patient.disabilityCertificate?.hasDisability ? '手帳' : '不要';
      case '年金':
        return patient.pensionStatus?.hasPension ? '年金' : '不要';
      default:
        return '-';
    }
  };
  ...
  const isSimultaneousApplicationPossible = (patient: any, type: DeadlineType) => {
    ...
    const selfSupport = patient.selfSupportCertificate;
    const disability = patient.disabilityCertificate;

    if (!selfSupport?.hasSupport || !disability?.hasDisability) return false;
    if (!selfSupport?.validUntil || !disability?.validUntil) return false;

    const selfSupportDate = new Date(selfSupport.validUntil);
    const disabilityDate = new Date(disability.validUntil);
  ...
  const deadlinePatients = useMemo(() => {
    ...
    activePatients.forEach((patient) => {
      selectedTypes.forEach((type) => {
        ...
        switch (type) {
          case '自立支援':
  deadline = patient.selfSupportCertificate?.validUntil;
  certificate = patient.selfSupportCertificate;
  hasRequirement = !!patient.selfSupportCertificate?.hasSupport;
  break;

case '障害者手帳':
  deadline = patient.disabilityCertificate?.validUntil;
  certificate = patient.disabilityCertificate;
  hasRequirement = !!patient.disabilityCertificate?.hasDisability;
  break;

case '年金':
  deadline = patient.pensionStatus?.validUntil;
  certificate = patient.pensionStatus;
  hasRequirement = !!patient.pensionStatus?.hasPension;
  break;
        ...
        if (year === selectedYear && (selectedMonths.size === 0 || selectedMonths.has(month))) {
          ...
          results.push({
            ...
            deadline,
            nextAction: getNextAction(certificate),
            certificateStatus: patient.medicalCertificateStates?.[certificateType]?.status || '未',
            notes: patient.notes,
